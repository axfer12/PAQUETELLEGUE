        ...
        for c in campos:
            ...
            if c in _int_campos:
                try:
                    ...
                    v = None
                # sucursal_id nunca debe ser NULL — fallback a 1
                if c == "sucursal_id" and v is None:
                    v = 1
            vals.append(v)
...
def get_guias(filtro="", fecha_ini=None, fecha_fin=None, operario_id=None, sucursal_id=None):
    ...
    print(f"[GET_GUIAS] operario_id={operario_id} sucursal_id={sucursal_id} params={params}", file=sys.stderr, flush=True)
    cur.execute(query, params)
    rows = cur.fetchall()
    print(f"[GET_GUIAS] resultado: {len(rows)} guias", file=sys.stderr, flush=True)
    conn.close()
    return [dict(r) for r in rows]
