...
def corte():
    ...
    # Filtro de operario — operarios ven toda la sucursal, supervisores pueden filtrar
    if current_user.is_supervisor:
        operario = request.args.get("operario", "")
    else:
        operario = ""  # Operarios ven todas las guías de su sucursal

    # Filtro de sucursal
    if current_user.is_admin_global:
        sucursal_filtro = request.args.get("sucursal", "")  # "" = todas
    else:
        sid_u = current_user.sucursal_id
        sucursal_filtro = str(sid_u) if sid_u else ""
...
def corte_raw():
    ...
    if current_user.is_supervisor:
        ...
        operario = ""  # Operarios ven toda la sucursal
    sid_u = current_user.sucursal_id
    sucursal_filtro_raw = "" if current_user.is_admin_global else (str(sid_u) if sid_u else "")
    ...
    if operario:
        query += f" AND g.operario_id = {ph}"; params.append(operario)
    if sucursal_filtro_raw:
        query += f" AND g.sucursal_id = {ph}"; params.append(sucursal_filtro_raw)
...
def corte_pdf():
    ...
    if current_user.is_supervisor:
        ...
        operario = ""  # Operarios ven toda la sucursal
    sid_u = current_user.sucursal_id
    sucursal_filtro_pdf = "" if current_user.is_admin_global else (str(sid_u) if sid_u else "")
    ...
    if operario:
        query += f" AND g.operario_id = {ph}"; params.append(operario)
    if sucursal_filtro_pdf:
        query += f" AND g.sucursal_id = {ph}"; params.append(sucursal_filtro_pdf)
    query += " ORDER BY g.creado_en"
    cur.execute(query, params)
    guias = [dict(r) if hasattr(r,'keys') else {
        'id':r[0],'numero_guia':r[1],'servicio':r[2],'destinatario_nombre':r[3],
