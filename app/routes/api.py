...
def cotizar():
    ...
    import sys
    print(f"[COTIZAR] cp_origen={d.get('cp_origen')} cp_destino={d.get('cp_destino')} pais_destino={d.get('pais_destino')}", flush=True, file=sys.stderr)
    try:
        rates = api.cotizar_envio(
            ...
            valor_declarado_usd=float(d.get("valor_declarado_usd") or 0),
        )
...
def generar_guia():
    ...
    try:
        ...
        envio = api.crear_envio(
            ...
            valor_declarado=float(d.get("valor_declarado") or paq.get("valor_declarado") or 1),
            valor_declarado_usd=float(d.get("valor_declarado_usd") or 0),
        ...
        import sys
        print(f"[VALOR] declarado={d.get('valor_declarado')} usd={d.get('valor_declarado_usd')} moneda deducida={'USD' if float(d.get('valor_declarado_usd') or 0) > 0 else 'MXN'}", file=sys.stderr, flush=True)
        ...
        if envio.get("pending"):
            ...
            return jsonify({"ok": True, "pending": True,
                            ...
                            "proveedor": "ei" if d.get("destinatario",{}).get("pais","MX").upper() != "MX" else "sky",
                            "_ctx": {
        ...
        return jsonify({"ok": True, "guia_id": guia_bd["id"] if guia_bd else None,
                        ...
                        "carrier": envio.get("carrier",""), "precio_final": precio_final,
                        "costo_seguro": costo_seguro,
                        "proveedor": "ei" if dest.get("pais","MX").upper() != "MX" else "sky"})
    except api.APIError as e:
        msg = str(e)
        _sin_saldo = any(k in msg.lower() for k in ("saldo insuficiente","balance","credito","crédito","credit","insufficient","funds","payment required","wallet"))
        if _sin_saldo:
...
def version():
    return jsonify({
        "version": "FINAL_MAY13e",
        "build": "2026-05-13T13:00",
        "fixes": ["sucursal_id_nunca_null", "ei_price_usd_directo", "historial_sucursal"]
    })

@bp.route("/debug_ei", methods=["POST"])
@login_required
def debug_ei():
    """Endpoint de debug — muestra exactamente qué se mandaría a EI."""
    d = request.get_json(force=True) or {}
    val_mxn = float(d.get("valor_declarado") or 1)
    val_usd  = float(d.get("valor_declarado_usd") or 0)
    # Replicar exactamente la lógica de cotizar_envio
    valor_cotiz = val_usd if val_usd and val_usd > 0 else val_mxn
    return jsonify({
        "recibido_mxn": val_mxn,
        "recibido_usd": val_usd,
        "precio_que_va_a_ei": valor_cotiz,
        "correcto": val_usd > 0 and val_usd == valor_cotiz
    })

@bp.route("/test_ei_precio")
@login_required
def test_ei_precio():
    """Test: cotiza en EI con price=10 y price=171 para ver cuál genera 171 en invoice."""
    import sys
    from app.modules import api_proveedor as api
    resultados = {}
    for precio in [10, 171.88]:
        try:
            rates = api.cotizar_envio(
                cp_origen="98830", cp_destino="49001",
                peso=1, alto=10, ancho=15, largo=20,
                pais_origen="MX", pais_destino="US",
                estado_origen="Zacatecas", ciudad_origen="Loreto",
                colonia_origen="Loreto Centro",
                estado_destino="Michigan", ciudad_destino="Kalamazoo",
                colonia_destino="NA",
                contenido="documentos",
                valor_declarado=precio if precio > 20 else 171.88,
                valor_declarado_usd=precio if precio <= 20 else 0,
            )
            resultados[f"price_{precio}"] = {
                "rates": len(rates),
                "carriers": [r.get("carrier") for r in rates[:3]],
                "import_duty": [r.get("arancel") for r in rates[:3]],
            }
        except Exception as e:
            resultados[f"price_{precio}"] = {"error": str(e)[:200]}
    return jsonify(resultados)

@bp.route("/tipo_cambio")
@login_required
def tipo_cambio():
    """Retorna el tipo de cambio USD/MXN actual."""
    try:
        import urllib.request, json as _json
        # Banxico API pública (no requiere key)
        url = "https://www.banxico.org.mx/SieAPIRest/service/v1/series/SF43718/datos/oportuno?token=abcdef"
        # Fallback: usar exchangerate-api gratuita
        r = urllib.request.urlopen("https://open.er-api.com/v6/latest/USD", timeout=3)
        d = _json.loads(r.read())
        mxn = d.get("rates",{}).get("MXN", 17.5)
        return jsonify({"ok": True, "usd_mxn": round(mxn, 4), "fuente": "exchangerate-api"})
    except Exception:
        # Fallback a tipo de cambio estático aproximado
        return jsonify({"ok": True, "usd_mxn": 17.5, "fuente": "fallback"})
@login_required
def buscar_clientes():
